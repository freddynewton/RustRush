...
namespace zone.nonon
{
    public class PatrolingState : State
    {
        ...
        private void Start()
        {
            enemy = mainTransform.GetComponent<Enemy>();
            enemyAgent = enemy.GetComponent<NavMeshAgent>();
        }

        public override State RunCurrentState()
        {
            Enemy.EnemyRayInfo rayInfo = enemy.SphereCast45();
            ...
            if (rayInfo != null)
            {
                if ((rayInfo.hitInfo.transform.tag.Equals(NononZoneConstants.HitBoxCollider.HITBOX_COLLIDED_TAG) &&
                    NononZoneObject.isOneOfTypes(rayInfo.hitInfo.transform.parent, INononZoneObject.NononZoneObjType.PLAYER)) ||
                    NononZoneObject.isOneOfTypes(rayInfo.hitInfo.transform, INononZoneObject.NononZoneObjType.PLAYER)
                    )
                {
                    ...
                    NononZoneObject no = playerTransform.GetComponent<NononZoneObject>();
