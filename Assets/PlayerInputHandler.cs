...
public class PlayerInputHandler : CharacterBehaviour
{
    [Header("Inventory")]
    ...
    /// Look Axis Values.
    /// </summary>
    private NetworkVariable<Vector2> axisLook_nw = new NetworkVariable<Vector2>();

    /// <summary>
    /// Look Axis Values.
    /// </summary>
    private NetworkVariable<Vector2> axisMovement_nw = new NetworkVariable<Vector2>();
    ...
    private bool CanRun()
    {
        ...
        if (axisMovement_nw.Value.y <= 0 || Math.Abs(Mathf.Abs(axisMovement_nw.Value.x) - 1) < 0.01f)
            return false;
    ...
    private void UpdateAnimator()
    {
        ...
        characterAnimator.SetFloat(HashMovement, Mathf.Clamp01(Mathf.Abs(axisMovement_nw.Value.x) + Mathf.Abs(axisMovement_nw.Value.y)), dampTimeLocomotion, Time.deltaTime);
    ...
    public override Vector2 GetInputMovement() => axisMovement_nw.Value;
    public override Vector2 GetInputLook() => axisLook_nw.Value;
    ...
    public void OnMove(InputAction.CallbackContext context)
    {
        ...
        if (IsClient && IsOwner)
        {
            axisMovement_nw.Value = cursorLocked ? context.ReadValue<Vector2>() : default;
            OnMovementInputServerRpc(axisMovement_nw.Value, axisMovement_nw.Value, axisMovement_nw.Value != Vector2.zero);
        }
...
//SetCurrentWalkMovement(_currentWalkMovement);
    ...
    {
        //Read.
        axisLook_nw.Value = cursorLocked ? context.ReadValue<Vector2>() : default;

        if (IsClient && IsOwner)
        {
            OnEnableRotationAndMoveCameraInputServerRpc(axisLook_nw.Value);
        }
    }
